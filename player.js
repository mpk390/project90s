...
class Player {
  ...
  update(dt,running,jumpBoost=1){this.time+=dt;if(this.mixer)this.mixer.update(dt);this.group.position.x=THREE.MathUtils.damp(this.group.position.x,this.targetX,13,dt);if(!this.grounded){this.yVelocity-=22*dt;this.group.position.y+=this.yVelocity*dt;if(this.group.position.y<=0){this.group.position.y=0;this.yVelocity=0;this.grounded=true;if(running)this.playAnimation('Run');}}
    ...
    if(!this.mixer&&running&&(this.model?.userData.rig||this.model?.userData.spriteCharacter))this.#proceduralRun(dt);
  ...
  #proceduralRun(){const t=this.time*11,s=Math.sin(t),c=Math.cos(t);const spriteData=this.model.userData.spriteCharacter;if(spriteData){spriteData.sprite.position.y=1.43+Math.abs(s)*.055;spriteData.sprite.material.rotation=c*.018;spriteData.shadow.scale.x=1.05-Math.abs(s)*.08;spriteData.shadow.material.opacity=.28-Math.abs(s)*.05;return;}const r=this.model.userData.rig;r.leftArm.rotation.x=s*.75;r.rightArm.rotation.x=-s*.75;r.leftLeg.rotation.x=-s*.72;r.rightLeg.rotation.x=s*.72;r.torso.rotation.z=c*.025;this.model.position.y=Math.abs(s)*.025;}
